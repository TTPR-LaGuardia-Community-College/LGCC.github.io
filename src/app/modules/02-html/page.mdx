...
- **[HTML Album Cover](https://github.com/ttpr-lgcc/html-album-cover-ttpr-002)** - Fork the repo, open `MOCKUP.jpg` next to your editor, and fill in `index.html` to recreate the album cover using only semantic HTML (headings, images, links, text). Filler copy lives in `SOURCE.md` so you focus on structure, not writing. No CSS — pure markup practice with a side-by-side visual target.

See also the [Assignments page](/assignments) for the central index.
